...
# 请求头
header = {
    # "Content-Type": "application/x-www-form-urlencoded", # 内容类型form-urlencoded
...
# 请求方式
# resp = requests.post(url, data=body, headers=header, timeout=2, verify=False) # 内容类型form-urlencoded
# resp = requests.post(url, json=body, headers=header, timeout=2, verify=False) # 内容类型json
resp = requests.post(url, params=params, json=body, headers=header, timeout=2, verify=False)  # 内容类型json

# 解码
jsonStr = str(resp.content, 'utf8')
print(jsonStr)